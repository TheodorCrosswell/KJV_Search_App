// Placeholder for all 66 books and their chapter counts
...
export async function getCycleStats(db) {
	...
	const counts = progress.map((p) => p.completion_counts);

	let globalMin = 0;
	if (counts.length === TOTAL_CHAPTERS) {
		globalMin = Math.min(...counts);
	}

	const completedInCycle = counts.filter((c) => c > globalMin).length;
